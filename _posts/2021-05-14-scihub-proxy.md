...
In the UK, many internet service providers block [Sci-Hub](https://en.wikipedia.org/wiki/Sci-Hub). Fortunately, a simple proxy is enough to circumvent this (you don't even need a VPN).
...
We can instruct the browser or operating system to read this file[^proxyWIN]. Search for instructions on Google ([example](https://www.google.com/search?q=proxy+auto+configuration+file+mac+os)).

[^proxyWIN]: One gotcha is that Windows 10 [stupidly forces you](https://docs.microsoft.com/en-us/troubleshoot/browsers/cannot-read-pac-file) to call your PAC file from a web server; it cannot be a local file. To work around this, you can upload your file as a [Gist](https://gist.github.com/) and link to the `/raw`.
